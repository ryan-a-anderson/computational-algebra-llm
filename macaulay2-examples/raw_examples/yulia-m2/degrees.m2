...
init = initialIdeal IEmpty
decomp = decompose init;
length decomp
netList decomp;
...
for el in decomp do {
    varz = flatten entries gens el;
    len = length unique(apply(varz, el -> ((baseName el)#1)_1));
    if (len == 8) then print(len, toString el);
    }
...
for el in decomp do {
    varz = flatten entries gens el;
    if toList (set({x_(1,1)}) * set varz) != {} then contain11 = append(contain11, el);
    }
